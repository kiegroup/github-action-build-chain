...
export class MockGithub {
  ...
  private async setBranches(
    ...
    pushedBranches.forEach((branch: string) => {
      promises.push(git.checkoutLocalBranch(branch).push("origin", branch, ["--set-upstream"]));
    });
      ...
      for (let j = 0; j < history.length; j++) {
        ...
        switch (hist.action.toLowerCase()) {
          case "push":
            ...
            await git.add(".").commit(`adding files to mimic history ${j}`).push(hist.remote, hist.branch);
            break;
  ...
  private getInterceptor(scope: nock.Scope, api: Api): nock.Interceptor {
    ...
    switch (api.method.toLowerCase()) {
      case "get":
        ...
        if (api.query) {
          mockedApi = mockedApi.query(api.query);
        } else {
          mockedApi = mockedApi.query(true);
        }
        break;
      case "post":
    ...
    for (let i = 0; i < this.config.mocks.api.length; i++) {
      ...
      api.response.forEach(res => {
        if (res.times) {
          ...
          scope = this.getInterceptor(scope, api).reply(res.code, res.data);
        }
        this.numNocks += 1;
      });
    }
  ...
  async setup() {
    ...
    if (fs.existsSync(this.setupPath)) {
      fs.rmSync(this.setupPath, { recursive: true });
    }
