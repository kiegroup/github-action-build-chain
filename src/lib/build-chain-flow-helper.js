...
const { clone, doesBranchExist, merge: gitMerge } = require("./git");
...
async function getCheckoutInfo(context, project, mapping) {
  ...
  const targetBranch =
    mapping && mapping.source === context.config.github.targetBranch
      ...
      : context.config.github.targetBranch;
  logger.info(
    `Getting checkout Info for ${project}. sourceGroup: ${sourceGroup}. sourceBranch: ${sourceBranch}. targetGroup: ${targetGroup}. targetBranch: ${targetBranch}. Mapping: ${mapping}`
  );
  return (await doesBranchExist(
    ...
    ? { group: sourceGroup, branch: sourceBranch, merge: true }
    ...
    ? { group: targetGroup, branch: sourceBranch, merge: true }
    : (await doesBranchExist(
        context.octokit,
        targetGroup,
        project,
