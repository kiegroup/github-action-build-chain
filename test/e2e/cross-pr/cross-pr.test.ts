...
test("PR from owner1/target:branchA to owner2/target:branchB while using mapping of the starting project (mapping.dependencies.X)", async () => {
  ...
  const result = await act
    ...
    .setEnv("CLONE_DIR", `${path.join(parentDir, "project1")} ${path.join(parentDir, "project2")} ${path.join(parentDir, "project3")}`)
    .setEvent({
      pull_request: {
        head: {
          ref: "branchA",
  ...
  // checking execution plan. making sure the correct level is taken for each project
  ...
  expect(group2.output).toEqual(expect.stringContaining("Level type: upstream"));
  ...
  // checking which project has been checked out and merged with with which branch. important to verify the mapped targets
  ...
  // before section for each project
  ...
  // current section for each project
  ...
  expect(group5.output).toEqual(expect.stringContaining("upstream owner1/project2"));
  ...
  // after section for each project
...
test("PR from target:branchA to target:branchB while using mapping of a non-starting project (mapping.dependant.X)", async () => {
  ...
  const result = await act
    ...
    .setEnv("CLONE_DIR", `${path.join(parentDir, "project1")} ${path.join(parentDir, "project2")}`)
    .setEvent({
      pull_request: {
        head: {
          ref: "branchA",
