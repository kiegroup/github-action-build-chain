const { readWorkflowInformation, getGroupAndBranchToCheckout, checkoutDependencies } = require("../src/lib/build-chain-flow-helper");
jest.mock('../src/lib/git');
const { doesBranchExist: doesBranchExistMock, clone: cloneMock } = require('../src/lib/git');
...
test("parseWorkflowInformation", () => {
  ...
  const expected = {
    ...
    'childDependencies': { 'appformer': { 'mapping': { 'source': '7.x', 'target': 'master' } }, 'lienzo-tests': {} },
...
test("checkoutDependencies", async () => {
  // Arrange
  const context = { 'config': { 'github': { 'workflow': 'main.yml', 'serverUrl': 'URL', 'author': 'author', 'sourceBranch': 'sBranch', 'group': 'group', 'targetBranch': 'tBranch' } } };
  const dependencies = { 'projectA': {}, 'projectB': { 'mapping': { 'source': 'tBranch', 'target': 'tBranchMapped' } }, 'projectC': {}, 'projectD': { 'mapping': { 'source': 'branchX', 'target': 'branchY' } } };
  doesBranchExistMock.mockResolvedValueOnce(true).mockResolvedValueOnce(false).mockResolvedValueOnce(false).mockResolvedValueOnce(true).mockResolvedValueOnce(false).mockResolvedValueOnce(false).mockResolvedValueOnce(true).mockResolvedValueOnce(false).mockResolvedValueOnce(false).mockResolvedValueOnce(true);
  // Act
  await checkoutDependencies(context, dependencies);
  // Assert
  expect(cloneMock).toHaveBeenCalledWith('URL/author/projectA', 'projectA', 'sBranch');
  expect(cloneMock).toHaveBeenCalledWith('URL/group/projectB', 'projectB', 'tBranchMapped');
  expect(cloneMock).toHaveBeenCalledWith('URL/group/projectC', 'projectC', 'tBranch');
  expect(cloneMock).toHaveBeenCalledWith('URL/group/projectD', 'projectD', 'tBranch');
});
